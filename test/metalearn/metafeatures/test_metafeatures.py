    ...
    def _test_compare_metafeature_lists(self):
        ...
        for filename, dataset in self.datasets.items():
            ...
            if os.path.exists(known_dataset_metafeatures_path):
                ...
                if len(computed_names_unique) > 0:
                    inconsistencies[known_dataset_metafeatures_path]["Computed Metafeatures"] = list(
                        computed_names_unique)
                if len(master_names_unique) > 0:
                    inconsistencies[known_dataset_metafeatures_path]["Master List Metafeatures"] = list(
                        master_names_unique)

        self.assertGreater(len(inconsistencies), 0,
                           "No known results could be loaded, metafeature lists could not be compared.")
        if not all(i == {} for i in inconsistencies.values()):
            ...
            self.assertTrue(False,
                            "Metafeature lists do not match, output written to {}.".format(inconsistency_report_file))
